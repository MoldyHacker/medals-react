...
const App = () => {
	...
	const apiEndpoint =
		"https://medals-api-ccm.azurewebsites.net/jwt/api/country"; // "https://localhost:7226/api/country"  "https://medals-api-6.azurewebsites.net/"
	...
	const usersEndpoint =
		"https://medals-api-ccm.azurewebsites.net/api/users/login";
	...
	const handleLogin = async (username, password) => {
		try {
			const resp = await axios.post(usersEndpoint, {
				username: username,
				password: password,
			});
	...
	const getUser = (encodedJwt) => {
		...
		return {
			...
			canPatch:
				decodedJwt["roles"].indexOf("medals-patch") === -1 ? false : true,
			canDelete:
				decodedJwt["roles"].indexOf("medals-delete") === -1 ? false : true,
		};
	};
	const handleLogout = () => {
